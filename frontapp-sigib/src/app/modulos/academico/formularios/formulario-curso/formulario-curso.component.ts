...
export class FormularioCursoComponent implements OnInit {
  ...
  private _inicializarFormulario() {
    this.formularioCurso = this._formBuilder.group({
      ...
      numeroMaximoAlumnos: [this.curso ? this.curso.numeroMaximoAlumnos : '', VALIDACION_MAXIMO_CURSO],
      profesor: [this.curso ? this.curso.profesor : '', VALIDACION_PROFESOR_CURSO],
      materia: [this.curso ? this.curso.materia : '', VALIDACION_MATERIA_CURSO],
    });
  }

  private _verificarCamposFormulario() {
    this.verificarCampoFormControl('horario', MENSAJES_VALIDACION_HORARIO_CURSO);
    ...
    this.verificarCampoFormControl('numeroMaximoAlumnos', MENSAJES_VALIDACION_MAXIMO_CURSO);
    this.verificarCampoFormControl('profesor', MENSAJES_VALIDACION_PROFESOR_CURSO);
    this.verificarCampoFormControl('materia', MENSAJES_VALIDACION_MATERIA_CURSO);
  }
      ...
      .subscribe(
        formulario => {
          ...
          if (cursoValidado) {
            ...
            console.log('asdfasdf', formulario);
  ...
  cargaMateriaProfesor() {
    this._materiaService.findAll()
      ...
      .subscribe((respuestaProfesores: [ProfesorInterface[], number]) => {
        this.profesor = respuestaProfesores[0];
        if (this.curso) {
          // tslint:disable-next-line:max-line-length
          this.indiceArregloMateria = this.curso.materia  ? this.materia.findIndex(materia => materia.id === (this.curso.materia as MateriaInterface).id) : -1;
          // tslint:disable-next-line:max-line-length
          this.indiceArregloProfesor = this.curso.profesor ? this.profesor.findIndex(profesor => profesor.id === (this.curso.profesor as ProfesorInterface).id) : -1;
        }
