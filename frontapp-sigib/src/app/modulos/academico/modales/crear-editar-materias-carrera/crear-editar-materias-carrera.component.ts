...
import { toastExitoEditar, toastErrorEditar, toastExitoCrear } from 'src/app/constantes/mensajes-toaster';
...
export class CrearEditarMateriasCarreraComponent implements OnInit {
  ...
  formularioValido;
  constructor(
    @Inject(MAT_DIALOG_DATA) public data: {materia: MateriaInterface, carrera: CarreraInterface},
...
nombre: materia.nombre,
  ...
  metodoCrearEditar() {
    ...
    if (this.data.materia) {
      this._materiaService
        ...
        .subscribe(
          r => {
            ...
            this._materiaService.findAll(JSON.stringify(consulta)).subscribe(
              res => {
      ...
      this.crearEditarMateria.codigo = this.crearEditarMateria.tipoMateria[0] + this.crearEditarMateria.anio[0];
      console.log('sadfsdfdfadsf', this.crearEditarMateria);
      this._materiaService
        .create(this.crearEditarMateria)
        .subscribe(
          r => {
            this._cargandoService.deshabilitarCargando();
